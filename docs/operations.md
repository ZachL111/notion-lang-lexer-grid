...
`notion-lang-lexer-grid` is centered on compilers work in Julia. The technical target is: Create a Julia reference implementation for lexer workflows, centered on protocol validation, framed sample traffic, and bounds and ordering tests.
...
The score is deterministic. The current threshold is 158. Risk has a penalty of 4, latency has a penalty of 2, and weight contributes a bonus of 2.
